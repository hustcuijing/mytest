...
function openGroupTypeAddDialog(){
		$('#type_adddlg').dialog("open").dialog("setTitle","添加群组类别信息");
		$('#name').removeAttr("readonly");
		url="mygrouptype?action=save";
	...
	function openGroupTypeModifyDialog(){
		...
		$("#type_adddlg").dialog("open").dialog("setTitle","修改群组类型信息");
		$("#fm").form("load",row);
		$('#name').removeAttr("readonly");
		url="mygrouptype?action=modify&typeid="+row.groupType_id;
	}
	function saveType(){
		$('#fm').form("submit",{
			url:url,
	...
	function deleteGroupType(){
		...
		for(var i=0;i<selectedRows.length;i++){
			strIds.push(selectedRows[i].groupType_id);
		}
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
			if(r){
				$.post("mygrouptype?action=delete",{delIds:ids},function(result){
					if(result.success){
...
<body style="margin: 1px;">
	...
	<div id="tb">
		...
		<div>
			<a href="javascript:openGroupTypeAddDialog()" class="easyui-linkbutton"
				...
				href="javascript:openGroupTypeModifyDialog()" class="easyui-linkbutton"
  ...
  <form id="fm" method="post">
	...
	<table id="mytable" title="群组类型列表" class="easyui-datagrid"
		pagination="true" fitColumns="false" rownumbers="true"  url="mygrouptype?action=list"
		fit="true"  toolbar="#tb">
		<thead>
			<tr>
				<th field="cb" checkbox="true" align="center"></th>
				<th field="groupType_id" width="100" align="center">群组编号</th>
